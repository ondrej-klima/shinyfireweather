...
#' @importFrom magrittr "%>%"
#'
MonthlyPoissonServer <- function(id, saved, data1, data2, data3, data4) {
  shiny::moduleServer(id, function (input, output, session) {
    ...
    observeEvent(saved$saved, ignoreInit = TRUE, {
      tryCatch({
        ...
        shiny::updateSelectInput(session, "dataChoice", selected = saved_input$input[["MonthlyPoisson-dataChoice"]])
        shiny::updateSelectInput(session, "var", selected = saved_input$input[["MonthlyPoisson-var"]])
        shiny::updateSelectInput(session, "date", selected = saved_input$input[["MonthlyPoisson-date"]])
        shiny::updateSelectInput(session, "area", selected = saved_input$input[["MonthlyPoisson-area"]])
        shiny::updateSelectInput(session, "areacode", selected = saved_input$input[["MonthlyPoisson-areacode"]])
        shiny::updateSelectInput(session, "confidenceLevels", selected = saved_input$input[["MonthlyPoisson-confidenceLevels"]])
      }, error = function(cond) {
      ...
      shiny::fluidRow(
        shiny::column(3,
                      isolate(
                        shiny::selectInput(
                          ...
                          selected = saved$saved$input[["MonthlyPoisson-dataChoice"]]
                        ))),
        shiny::column(3,
        ...
        shiny::column(3,
                      shiny::selectInput(
                        ...
                        selected = saved$saved$input[["MonthlyPoisson-confidenceLevels"]]
                      )),
        shiny::column(3,
                      shiny::HTML("&nbsp;<br />"),
                      shiny::actionButton(
    ...
    observeEvent(input$dataChoice, {
      ...
      output$varUi <- shiny::renderUI(
        shiny::selectInput(
          ...
          selected = saved$saved$input[["MonthlyPoisson-var"]]
        )
      )
      ...
      output$dateUi <- shiny::renderUI(
        shiny::selectInput(
          ...
          choices = colnames(data()),
          selected = saved$saved$input[["MonthlyPoisson-date"]]
        )
      )
      ...
      output$areaColUi <- shiny::renderUI(
        shiny::selectInput(
          ...
          choices = colnames(data()),
          selected = saved$saved$input[["MonthlyPoisson-area"]]
        )
      )
      }, error = function(cond) {
        shiny::showNotification(conditionMessage(cond), type="error")
    ...
    observeEvent(input$area, {
      ...
      output$areaui <- renderUI({
        shiny::selectInput(shiny::NS(id,"areacode"), "Kraj",
                           ...
                           selected = saved$saved$input[["MonthlyPoisson-areacode"]])
      })
      }, error = function(cond) {
        shiny::showNotification(conditionMessage(cond), type="error")
        NA
