...
#' @importFrom magrittr "%>%"
#'
MonthlyQuasiPoissonServer <- function(id, saved, data1, data2, data3, data4) {
  shiny::moduleServer(id, function (input, output, session) {
    ...
    observeEvent(saved$saved, ignoreInit = TRUE, {
      tryCatch({
        ...
        shiny::updateSelectInput(session, "dataChoice", selected = saved_input$input[["MonthlyQuasiPoisson-dataChoice"]])
        shiny::updateSelectInput(session, "var", selected = saved_input$input[["MonthlyQuasiPoisson-var"]])
        shiny::updateSelectInput(session, "date", selected = saved_input$input[["MonthlyQuasiPoisson-date"]])
        shiny::updateSelectInput(session, "area", selected = saved_input$input[["MonthlyQuasiPoisson-area"]])
        shiny::updateSelectInput(session, "areacode", selected = saved_input$input[["MonthlyQuasiPoisson-areacode"]])
        shiny::updateSelectInput(session, "confidenceLevels", selected = saved_input$input[["MonthlyQuasiPoisson-confidenceLevels"]])
      }, error = function(cond) {
      ...
      shiny::fluidRow(
        shiny::column(3,
                      isolate(
                        shiny::selectInput(
                          ...
                          selected = saved$saved$input[["MonthlyQuasiPoisson-dataChoice"]]
                        ))),
        shiny::column(3,
        ...
        shiny::column(3,
                      shiny::selectInput(
                        ...
                        selected = saved$saved$input[["MonthlyQuasiPoisson-confidenceLevels"]]
                      )),
        shiny::column(3,
                      shiny::HTML("&nbsp;<br />"),
                      shiny::actionButton(
    ...
    observeEvent(input$dataChoice, {
      ...
      output$varUi <- shiny::renderUI(
        shiny::selectInput(
          ...
          selected = saved$saved$input[["MonthlyQuasiPoisson-var"]]
        )
      )
      ...
      output$dateUi <- shiny::renderUI(
        shiny::selectInput(
          ...
          choices = colnames(data()),
          selected = saved$saved$input[["MonthlyQuasiPoisson-date"]]
        )
      )
      ...
      output$areaColUi <- shiny::renderUI(
        shiny::selectInput(
          ...
          choices = colnames(data()),
          selected = saved$saved$input[["MonthlyQuasiPoisson-area"]]
        )
      )
      }, error = function(cond) {
        shiny::showNotification(conditionMessage(cond), type="error")
    ...
    observeEvent(input$area, {
      ...
      output$areaui <- renderUI({
        shiny::selectInput(shiny::NS(id,"areacode"), "Kraj",
                           ...
                           selected = saved$saved$input[["MonthlyQuasiPoisson-areacode"]])
      })
      }, error = function(cond) {
        shiny::showNotification(conditionMessage(cond), type="error")
        NA
